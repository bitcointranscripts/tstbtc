...
class DataWriter:
    ...
    def write_json(self, data, file_path, filename):
        """
        ...
        output_file = self.construct_file_path(
            file_path, filename, type='json')
        with open(output_file, 'w') as json_file:
            json.dump(data, json_file, indent=4)
        return output_file

    def construct_file_path(self, file_path, filename, type: Literal['json', 'srt']):
        """
        ...
        filename = f"{self.add_timestamp(filename)}.{type}"
